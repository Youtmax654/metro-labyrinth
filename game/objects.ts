...
class Labyrinth {
  ...
  constructor(scene: THREE.Scene, dimension: number, tileOffset = 1) {
    ...
    this.tiles[this.maxDim][this.maxDim].directions = [
      Direction.RIGHT,
      Direction.DOWN,
    ];
    ...
    let randomTreasures = shuffle(availableTreasureRandomSlots).slice(
      0,
      this.totalTreasures / 2
    );

    for (let t of randomTreasures)
      everyTreasuresCoords.push({ x: t.x, y: t.y });

    const treasuresPerPawn = Math.round(
      this.totalTreasures / this.pawns.length
    );
    for (let i = 0; i < this.pawns.length; ++i) {
      let pawnTreasures = shuffle(everyTreasuresCoords).splice(
        0,
        treasuresPerPawn
      );
      for (let j = 0; j < pawnTreasures.length; ++j) {
        ...
        this.tiles[pawnTreasures[j].x][pawnTreasures[j].y].treasureId =
          treasureId;
        treasureId++;
      }
    ...
    for (let i = 0; i < this.pawns.length; ++i) {
      for (let j = 0; j < this.pawns[i].remainingTreasures.length; ++j) {
        ...
        (this.treasures[id].mesh.material as THREE.MeshBasicMaterial).color = (
          this.pawns[i].mesh.material as THREE.MeshBasicMaterial
        ).color;
      }
      (
        this.treasures[this.pawns[i].remainingTreasures[0]].mesh
    ...
    let quotaRandomTiles = {
      [TileType.STRAIGHT]: Math.round(
        ((this.dimension * this.dimension) / 28) * this.dimension
      ),
      [TileType.CORNER]: Math.round(
        ((this.dimension * this.dimension) / 21) * this.dimension
      ),
      [TileType.TJUNCTION]: Math.round(
        ((this.dimension * this.dimension) / 57) * this.dimension
      ),
    };
      ...
      case Direction.DOWN:
        {
          ...
          for (let i = this.maxDim; i >= 0; i--) {
            this.moveTiles(this.selectedLaneX, i, this.selectedLaneX, i - 1);
            [
              this.tiles[this.selectedLaneX][this.maxDim],
              this.tiles[this.selectedLaneX][i],
            ] = [
              this.tiles[this.selectedLaneX][i],
              this.tiles[this.selectedLaneX][this.maxDim],
      ...
      case Direction.RIGHT:
        {
          ...
          for (let i = this.maxDim; i >= 0; i--) {
            this.moveTiles(i, this.selectedLaneY, i - 1, this.selectedLaneY);
            [
              this.tiles[this.maxDim][this.selectedLaneY],
              this.tiles[i][this.selectedLaneY],
            ] = [
              this.tiles[i][this.selectedLaneY],
              this.tiles[this.maxDim][this.selectedLaneY],
  ...
  checkPlayerTreasures(currentPawn: number, window: Window) {
    ...
    if (
      seeked.x == this.pawns[currentPawn].x &&
      ...
      if (this.pawns[currentPawn].remainingTreasures.length > 0) {
        ...
        this.pawns[currentPawn].remainingTreasures[0] =
          this.pawns[currentPawn].remainingTreasures[0];
        (
          this.treasures[this.pawns[currentPawn].remainingTreasures[0]].mesh
            .material as THREE.MeshBasicMaterial
        ).wireframe = true;
      } else {
...
class OrbitCamera {
  ...
  constructor(
    ...
    this.controller.maxDistance =
      labyrinth.dimension * labyrinth.tileOffset + 1;
...
class Game {
  ...
  constructor(window: Window) {
    ...
    this.currentPawn = 0;
    (
      this.labyrinth.pawns[this.currentPawn].mesh
        .material as THREE.MeshBasicMaterial
    ).wireframe = true;
  ...
  getPlayerTile() {
    return this.labyrinth.tiles[this.labyrinth.pawns[this.currentPawn].x][
...
export { Direction, Game, GamePhase, LaneAxis };
